...
class StudentControllerTest {
  ...
  @Test
  void getStudentList_受講生の一覧情報が検索できて返ってくること() throws Exception {
...
//        .andExpect(content().json("[]"));
  ...
  @Test
  void getStudent_受講生詳細の受講生で正常な値を設定したときにエラーにならないこと() {
...
//    Assertions.assertEquals(0, violations.size());
  ...
  @Test
  void getStudent_受講生詳細の受講生で電話番号が不正なときにチェックにひっかかること() {
    Student student = new Student();
    student.setFurigana("satou");
    student.setRegion("大阪");
    student.setEmail("sample@gmail.com");
...
//    Assertions.assertEquals(0, violations.size());
  ...
  @Test
  void getStudent_正常なIDで受講生情報が取得できること() throws Exception {
  ...
  @Test
  void getStudent_異常なIDでバリデーションエラーが発生すること() throws Exception {
    mockMvc.perform(get("/students/1000")) // 1000は @Max(999) を超えているため異常値
        .andExpect(status().isBadRequest());
  }

  @Test
  void resisterStudent_正常なデータをPOSTしたときに成功すること() throws Exception {
...
//    studentDetail.getStudent().setId(1);
  ...
  @Test
  void updateStudent_正常なデータをPUTしたときに成功すること() throws Exception {
  ...
  //以下バリデーションチェック
  @Test
  void testValidation_NameIsEmpty_ShouldFail() {
    Student student = createValidStudent();
  ...
  @Test
  void testValidation_InvalidPhoneNumber_ShouldFail() {
    Student student = createValidStudent();
  ...
  @Test
  void testValidation_FuriganaIsNull_ShouldFail() {
    Student student = createValidStudent();
  ...
  @Test
  void testValidation_RegionIsNull_ShouldFail() {
    Student student = createValidStudent();
  ...
  @Test
  void testValidation_NicknameIsNull_ShouldFail() {
    Student student = createValidStudent();
